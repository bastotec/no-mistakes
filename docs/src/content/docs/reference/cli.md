...
| Retire | `axi run --retire-existing-pr` | Removes the record and starts no run; later runs use ordinary discovery. It takes no other flag - it starts nothing, so a launch flag beside it would be discarded |
