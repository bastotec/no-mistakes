...
This is per-run, not repository configuration. It cannot be combined with `--skip`. A matching active run can be reattached to without changing its pins, and no other launch supersedes an active protected run: `axi run` refuses before pushing whenever the caller is not at that run's head, and a push that reaches the gate directly is refused with the command that restores the gate branch to the run's recorded head. With explicit receipt flags, replay requires the same pins and intent, and a new nonce refuses an already-active branch. `rerun` cannot implicitly repin a protected run: recover custody if necessary, explicitly prepare the desired integration, and start a new constrained run. Ordinary callers without the flag keep their existing behavior.
