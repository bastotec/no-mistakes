...
On a branch associated with an [explicit existing upstream PR](/no-mistakes/reference/cli/#explicit-existing-upstream-pr), that pull request's own base branch replaces this setting for every step, and `--base-branch` is refused.
