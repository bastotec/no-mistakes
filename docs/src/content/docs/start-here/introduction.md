...
- A fixed, opinionated core pipeline: `intent → rebase → review → jev* → test → document → lint → push → pr → ci` (`jev*` is the advisory-only evaluation signal, disabled by default). Repositories can add [command gates](/no-mistakes/reference/repo-config/#gates) after selected core steps, but cannot remove or reorder those steps.
- Choice of [native or ACP agent](/no-mistakes/reference/global-config/#agent), with per-repo override and ordered fallbacks; every gate requires a runnable configured pipeline agent.
