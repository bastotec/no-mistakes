...
Passing `VERSION=` by hand overrides that, and the build refuses a value with no version number in it (`VERSION=fork-7e84d0d`, `VERSION=dev`). A version string with nothing comparable in it makes every minimum-version check read the tool as not installed and offer to install over the build that is running. To mark a fork, keep the marker beside a real version using semantic versioning's build metadata - `make build VERSION=v1.76.0+fork.7e84d0d` - rather than in place of it: the released version this build comes from, plus the commit that identifies your build. Build metadata rather than a prerelease suffix such as `v1.76.0-5-g7e84d0d+fork`, because semver ranks a prerelease below the release it names, so a prerelease-shaped fork reads as older than the `v1.76.0` it was built from and the updater offers to replace it with that older release. The refusal prints a suggestion of that shape.
