...
`make build` stamps the version `no-mistakes version` reports. A checkout sitting exactly at a release tag, with a clean tree, stamps that tag verbatim - the release path. Any other build stamps the fork stamp `<upstream-semver>-fork-<sha>`, for example `1.83.1-fork-6911e03`: the newest release tag the clone knows (the upstream semantic version the fork tracks), then the fork marker, then the build's short commit. The comparable core passes every minimum-version check, so a from-source build proves its version floor instead of reporting `dev` and inviting an install over itself, and the whole stamp marks the binary as this fork's build rather than stock upstream. Outside a checkout, or in a shallow or tagless clone where no release tag can be found, it falls back to the `dev` sentinel.
