...
`jev*` is the advisory-only evaluation signal: disabled unless the operator enables it in global config, it never gates and skips (with a reason) whenever it is unconfigured or unavailable.

```mermaid
flowchart LR
  intent["Intent"] --> rebase["Rebase"] --> review["Review"] --> jev["Jev*"] --> test["Test"] --> document["Document"] --> lint["Lint"] --> push["Push"] --> pr["PR"] --> ci["CI"]
...
| 4 | **Jev** | Advisory-only evaluation of the diff (disabled by default; never gates) | n/a |
...
- **Jev after review** so an independent evaluation model reads the same freshly-reviewed diff - but only as a reported hint: it is disabled by default, never gates, and never fixes.
