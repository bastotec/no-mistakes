...
func registerHandlers(srv *ipc.Server, mgr *RunManager, d *db.DB, shutdown func()) {
	...
	srv.Handle(ipc.MethodStartHistoryRun, func(ctx context.Context, params json.RawMessage) (interface{}, error) {
		if err := refuseNested(ctx, false); err != nil {
			return nil, err
		}
		var p ipc.StartHistoryRunParams
		if err := json.Unmarshal(params, &p); err != nil {
			return nil, fmt.Errorf("invalid params: %w", err)
		}
		receipt, err := mgr.HandleStartHistoryRun(ctx, &p)
		if err != nil {
			return nil, err
		}
